...
export function isSimplifiedChineseLocale(locale: string): boolean {
  ...
  if (normalized === 'zh') return true;
  if (!normalized.startsWith('zh-')) return false;
  if (normalized.includes('hant') || normalized.startsWith('zh-tw') || normalized.startsWith('zh-hk') || normalized.startsWith('zh-mo')) return false;
  return normalized.includes('hans') || normalized.startsWith('zh-cn') || normalized.startsWith('zh-sg');
}

try {
  const saved = window.localStorage.getItem(STORAGE_KEY);
  if (saved === 'zh-CN') language = 'zh-CN';
  else if (saved !== 'en' && window.navigator.languages.some(isSimplifiedChineseLocale)) language = 'zh-CN';
} catch {
  // Preserve the fork's browser-locale default even if storage is unavailable.
