...
describe('Chinese app interface', () => {
  it('auto-selects only Simplified Chinese browser locales when no preference is saved', async () => {
    Object.defineProperty(window.navigator, 'languages', { value: ['zh-CN', 'en-US'], configurable: true });
    const simplified = await import('../src/renderer/i18n.js');
    expect(simplified.isSimplifiedChineseLocale('zh')).toBe(true);
    expect(simplified.isSimplifiedChineseLocale('zh-Hans-SG')).toBe(true);
    expect(simplified.isSimplifiedChineseLocale('zh-TW')).toBe(false);
    expect(simplified.isSimplifiedChineseLocale('zh-Hant-HK')).toBe(false);
    expect(simplified.currentLanguage()).toBe('zh-CN');
  });
