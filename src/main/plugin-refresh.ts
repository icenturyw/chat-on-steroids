...
async function rows(): Promise<Row[]> {
  const result = z.array(rowSchema).max(2).parse(await readDurable('plugin-refresh') ?? []);
...
function recognizable(tools: unknown): tools is PluginToolSchema[] {
  if (!Array.isArray(tools) || !tools.length || tools.length > 16 || JSON.stringify(tools).length > 300000) return false;
...
function enrollable(tools: unknown, publication: PluginPublication): boolean {
  if (!recognizable(tools)) return false;
...
function matches(tools: unknown, expected: PluginToolSchema[]): boolean {
  return recognizable(tools) && hash(declaration(tools)) === hash(declaration(expected));
}
...
export function claimPluginRefresh(input: Identity & { connectorName: string; tools: unknown; alreadyCurrent?: boolean }): Promise<boolean> {
  return serial(async () => {
    ...
    const isCurrent = matches(input.tools, publication.tools);
