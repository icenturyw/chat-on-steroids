...
import { dispatch, fail, transportConversationOf, type ToolResult } from './kernel.js';
...
export function registerPluginTools(server: McpServer): PluginToolSchema[] {
  ...
  server.server.setRequestHandler('tools/call', async (request, context) => {
    ...
    const transportConversation = transportConversationOf(context);
    const transportKey = transportConversation ?? context.sessionId ?? null;
    const result = await dispatch(request.params.name, request.params.arguments ?? {}, transportKey,
      transportConversation, inboundRequestId(), 'plugins', async () => {
        // An external annotation is informational, never proof a process cannot mutate.
        if (getConfig().readOnly) return fail('TOOL_DISABLED: external plugins are unavailable while CoS read-only mode is on.');
